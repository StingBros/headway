...
| Auto timeline | A phase can be put on **Auto timeline** (phase context menu): its features are laid out by dependency order at the earliest start with free capacity, bars stretch across holiday weeks, locked and done items stay put, and the layout re-runs after every change. For a single item, **Place at earliest slot** / **Snap earliest** from the row's context menu or the panel |
