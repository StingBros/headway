...
(function (root) {
  ...
  AI.summary = function (state) {
    ...
    return {
      ...
      phases: state.phases.map(function (p) {
        return { id: p.id, name: p.name, bucket: !!p.bucket, auto: !!p.auto, items: counts[p.id] || 0,
  ...
  AI.TOOLS = [
    ...
    {
      name: 'update_project',
      description: 'Edit any other part of the document with path operations, e.g. project settings (meta/title, meta/vision, meta/timelineStart, meta/endDate, meta/weeksPerSprint, meta/sprintAnchor, meta/sprintAnchorNum, meta/workDays, meta/sizeScheme, meta/sizeDays/M, meta/priorityScheme, meta/storyPriorityScheme, meta/riskScheme, meta/capacityEnabled, meta/capMode ("person"|"points"), meta/defaultPoints, meta/planLevel, phases/@id/auto (Auto timeline), meta/holidayRanges (push {name,start,end}), meta/scopeCols (push {key:"c<slug>", label}), meta/jira, meta/itemTypes (array of {key,label,icon,jira}), meta/hierarchy/levels/<i>/types, meta/hierarchy/anyTypeAnyLevel, epicTypes/<name>), phases (phases/@id/name, phases/- to append {name, bucket}), team (team/@id/rate, team/- to append {name, role, type, workstreams, capacity, rate, cost}), teamTypes, wsColors/<name>, epicIcons/<name> (lucide icon), epicJira/<name>, wsOrder. Path segments: #num = feature by number, @id = element by id, digits = index, "-" = append. Ops: set (path, value), delete (path), push (path, value). Prefer add_items / update_items for features and stories.',
      parameters: {
        type: 'object',
        properties: {
  ...
  AI.GUIDE = [
    ...
    '- Team (state.team): people or seats with role, rate-card type, workstreams, capacity (heads at 40 h; 0.5 = half-time), hourly rate and cost, weekHours overrides, capType = what they supply; points = story points per sprint (points mode). Capacity checks only run when meta.capacityEnabled; phases with auto=true are re-laid out by dependencies and capacity after every change.',
